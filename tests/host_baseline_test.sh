...
# Historical host alias resolves to the original pair with unchanged metrics.
historical=5e6e762c7322034ba38e814aa0aaa8fe
[[ $(perf_select_baseline "$repo_root/performance-logs/baselines" "$a" "$historical") == "$repo_root/performance-logs/baselines/$historical" ]] || fail historical
printf 'row ns/op=100\n' > "$scratch/base"
