...
"CMAKE_CROSSCOMPILING:BOOL=FALSE\n")
execute_process(
    COMMAND "${PSLOG_ROOT}/scripts/discover_target_tools.sh"
...
set(native_darwin_build "${test_root}/native-darwin-build")
set(native_darwin_bin "${test_root}/native-darwin-bin")
file(MAKE_DIRECTORY "${native_darwin_build}" "${native_darwin_bin}")
foreach(tool IN ITEMS cc)
    fake_tool("${native_darwin_bin}/${tool}")
endforeach()
file(WRITE "${native_darwin_build}/CMakeCache.txt"
"CMAKE_C_COMPILER:FILEPATH=${native_darwin_bin}/cc\n"
"CMAKE_CROSSCOMPILING:BOOL=FALSE\n"
"CMAKE_STRIP:FILEPATH=/usr/bin/strip\n"
"CPKT_OTOOL:FILEPATH=/usr/bin/strip\n")
execute_process(
    COMMAND "${PSLOG_ROOT}/scripts/discover_target_tools.sh"
        --build-dir "${native_darwin_build}" --target-id arm64-apple-darwin
    RESULT_VARIABLE native_darwin_result
    OUTPUT_VARIABLE native_darwin_output
    ERROR_VARIABLE native_darwin_error
)
if(NOT native_darwin_result EQUAL 0 OR
   NOT native_darwin_output MATCHES "STRIP=/usr/bin/strip" OR
   NOT native_darwin_output MATCHES "OTOOL=/usr/bin/strip")
    message(FATAL_ERROR "native Darwin discovery rejected host tools:\n${native_darwin_output}\n${native_darwin_error}")
endif()
