...
for command in cmake ctest qemu-aarch64 qemu-arm; do
...
chmod +x "$fake_repo/scripts/run_timed.sh" "$fake_repo/lua/scripts/release_version.sh" \
  "$fake_repo/scripts/verify_release_privacy.sh" "$fake_bin/env" "$fake_bin/dirname" "$fake_bin/make" "$fake_bin/custom-luarocks"
