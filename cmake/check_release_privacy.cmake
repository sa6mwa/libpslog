...
foreach(variable CPKT_TOOLCHAIN_CACHE CPKT_DEPENDENCY_CACHE XDG_CACHE_HOME)
    if(DEFINED ENV{${variable}} AND NOT "$ENV{${variable}}" STREQUAL "")
        list(APPEND forbidden_literals "$ENV{${variable}}")
    endif()
endforeach()
if(PSLOG_READELF MATCHES "^(.+)/roots/[^/]+/bin/[^/]+$")
    list(APPEND forbidden_literals "${CMAKE_MATCH_1}")
endif()
...
include("${CMAKE_CURRENT_LIST_DIR}/pslog_elf_privacy.cmake")
...
function(pslog_check_extracted_tree tree_root context)
    ...
    foreach(entry IN LISTS extracted_entries)
        ...
        elseif(entry_name MATCHES "\\.(tar\\.gz|tgz|tar\\.xz|zip|rock)$")
...
foreach(artifact IN LISTS privacy_paths)
    ...
    if(artifact_name MATCHES "\\.(tar\\.gz|tgz|tar\\.xz|zip|rock)$")
