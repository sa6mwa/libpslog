...
crosscompiling=$(cache_value CMAKE_CROSSCOMPILING)
case "$crosscompiling" in
  TRUE|True|true|ON|On|on|1|YES|Yes|yes) crosscompiling=true ;;
...
is_known_host_tool() {
  local tool=$1
  [[ "$crosscompiling" == true && "$tool" =~ ^/usr/bin/(strip|readelf|otool|install_name_tool)$ ]]
}
